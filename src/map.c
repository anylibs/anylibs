...
static bool               c_internal_map_resize(CMap* self, size_t new_capacity);
...
static inline CMapBucket* c_internal_map_get_bucket(CMap const* self, size_t index);

CMap* c_map_create(size_t key_size, size_t value_size, CAllocator* allocator)
{
  return c_map_create_with_capacity(key_size, value_size, CMAP_DEFAULT_CAPACITY, allocator);
}
...
CMap* c_map_create_with_capacity(size_t key_size, size_t value_size, size_t capacity, CAllocator* allocator)
{
  ...
  CMap* map = c_allocator_alloc(allocator, c_allocator_alignas(CMap, 1), true);
  if (!map) return NULL;
  ...
  size_t bucket_size = sizeof(CMapBucket) + aligned_key_size + aligned_value_size;

  map->spare_bucket1 = c_allocator_alloc(allocator, (capacity + spare_buckets_count) * bucket_size, sizeof(CMapBucket), true);
...
{
  void* out_value;
  bool  status = c_map_get(self, key, &out_value);
...
bool c_map_remove(CMap* self, void* key, void** out_value)
{
  ...
  for (size_t index = hash & self->mask;; index = (index + 1) & self->mask) {
    ...
    if (bucket->hash == hash && (memcmp(bucket_key, key, self->key_size.orig) == 0)) {
      ...
      *out_value = (char*)self->spare_bucket1 + sizeof(CMapBucket) + self->key_size.aligned;
      break;
    }
  }
...
void c_map_clear(CMap* self, CMapElementDestroyFn element_destroy_fn, void* user_data)
{
  if (element_destroy_fn) {
    void*    key;
    void*    value;
    CMapIter iter = c_map_iter(self);
    while (c_map_iter_next(&iter, &key, &value)) {
...
bool c_map_iter_next(CMapIter* iter, void** key, void** value)
{
  ...
  for (; iter->index < iter->map->capacity; ++iter->index) {
    CMapBucket* bucket = c_internal_map_get_bucket(iter->map, iter->index);
    if (bucket->distance_from_initial_bucket) {
...
bool c_internal_map_resize(CMap* self, size_t new_capacity)
{
  CMap* new_map = c_map_create_with_capacity(self->key_size.orig, self->value_size.orig, new_capacity, self->allocator);
  ...
  for (size_t iii = 0; iii < self->capacity; iii++) {
    ...
    for (size_t new_index = bucket->hash & new_map->mask;; new_index = (new_index + 1) & new_map->mask) {
      CMapBucket* new_map_bucket = c_internal_map_get_bucket(new_map, new_index);
...
void* c_internal_map_get_key(const CMap* self, size_t index)
{
  return (void*)(((char*)(c_internal_map_get_bucket(self, index))) + sizeof(CMapBucket));
}

void* c_internal_map_get_value(const CMap* self, size_t index)
{
  return (void*)(((char*)(c_internal_map_get_bucket(self, index))) + sizeof(CMapBucket) + self->key_size.aligned);
}

CMapBucket* c_internal_map_get_bucket(const CMap* self, size_t index)
{
